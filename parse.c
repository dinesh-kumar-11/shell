...
parseInfo *parse (char *cmdline) {
  ...
  while (cmdline[i] != '\n' && cmdline[i] != '\0') {
	...
	fprintf(stderr, "Ignore anything beyond &.\n");
      ...
      pos=0;
      while (cmdline[i] != '\0' && !isspace(cmdline[i]) && cmdline[i] != '>') {
...
void print_info (parseInfo *info) {
  ...
  if( info->boolMultipleCmd ) {
	  printf("Multiple Command to be executed.\n");
  }else {
	  printf("Single Command to be executed.\n");
  }
